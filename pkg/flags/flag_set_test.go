...
func ValidateFlags(t *testing.T, fs ff.Flags, args []string, options ...ff.Option) {
	t.Helper()
	...
	if err := ff.Parse(fs, args, options...); err != nil {
...
func TestFlagSet_Basics(t *testing.T) {
	...
	for _, argstr := range []string{
		...
		t.Run(argstr, func(t *testing.T) {
			...
			ValidateFlags(t, fs, strings.Fields(argstr))
		})
	}
}
...
func TestFlagSet_Bool(t *testing.T) {
	...
	t.Run("add bool flag", func(t *testing.T) {
		var (
			fs     = flags.NewFlagSet(t.Name())
			bflag  bool
			bvalue = ffval.NewValueDefault(&bflag, true)
		)

		if _, err := fs.AddFlag(flags.FlagConfig{
			ShortName: 'b',
			Value:     bvalue,
		}); err == nil {
	...
	for _, test := range []struct {
		...
		{args: []string{"--xflag", "true"}, wantX: true, wantY: true},
		{args: []string{"-x=true"}, wantX: false, wantY: true, wantErr: flags.ErrUnknownFlag}, // = interpreted as flag
		...
		{args: []string{"-y=false"}, wantX: false, wantY: false, wantErr: flags.ErrUnknownFlag}, // = interpreted as flag
		{args: []string{"-h"}, wantX: false, wantY: true, wantErr: flags.ErrHelp},
		{args: []string{"--help"}, wantX: false, wantY: true, wantErr: flags.ErrHelp},
		{args: []string{"--xflag", "-h"}, wantX: true, wantY: true, wantErr: flags.ErrHelp},
		{args: []string{"-y", "--help"}, wantX: false, wantY: false, wantErr: flags.ErrHelp},
	} {
...
func TestStdFlags_Bool(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{args: []string{"--help"}, wantX: false, wantY: true, wantErr: flags.ErrHelp},
		{args: []string{"-xflag", "-h"}, wantX: true, wantY: true, wantErr: flags.ErrHelp},
		{args: []string{"--y=false", "--help"}, wantX: false, wantY: false, wantErr: flags.ErrHelp},
	} {
		t.Run(strings.Join(test.args, " "), func(t *testing.T) {
...
func TestFlagSet_HelpFlag(t *testing.T) {
	...
	if err := fs.Parse([]string{"-h"}); !errors.Is(err, flags.ErrHelp) {
		t.Errorf("Parse(-h): want %v, have %v", flags.ErrHelp, err)
	}
...
func TestFlagSet_NoDefault(t *testing.T) {
	...
	fs := flags.NewFlagSet(t.Name())
	alpha, _ := fs.AddFlag(flags.FlagConfig{LongName: "alpha", Value: &ffval.Duration{}, Usage: "zero duration"})
	beta, _ := fs.AddFlag(flags.FlagConfig{LongName: "beta", Value: &ffval.Duration{}, Usage: "zero duration with NoDefault", NoDefault: true})
...
func TestFlagSet_NoPlaceholder(t *testing.T) {
	...
	fs := flags.NewFlagSet(t.Name())
	alpha, _ := fs.AddFlag(flags.FlagConfig{LongName: "alpha", Value: &ffval.Bool{}, Usage: "alpha", NoPlaceholder: true})
	beta, _ := fs.AddFlag(flags.FlagConfig{LongName: "beta", Value: ffval.NewValueDefault(new(bool), true), Usage: "beta", NoPlaceholder: true})
	delta, _ := fs.AddFlag(flags.FlagConfig{LongName: "delta", Value: ffval.NewValueDefault(new(bool), true), Usage: "delta `D` flag", NoPlaceholder: true})
	kappa, _ := fs.AddFlag(flags.FlagConfig{LongName: "kappa", Value: ffval.NewValue(new(bool)), Usage: "kappa `K` flag", NoPlaceholder: true})
...
func TestFlagSet_Get(t *testing.T) {
	...
	fs := flags.NewFlagSet(t.Name())
	f, err := fs.AddFlag(flags.FlagConfig{
		LongName:    "alpha",
		Value:       new(ffval.Int),
		Placeholder: "X",
	})
...
func TestFlagSet_structs(t *testing.T) {
	...
	var inflags myFlags
	fs := flags.NewFlagSetFrom(t.Name(), &inflags)
	...
	for _, testcase := range []struct {
		...
		t.Run(testcase.args, func(t *testing.T) {
			...
			if want, have := testcase.want, inflags; !reflect.DeepEqual(want, have) {
				t.Errorf("\nwant %+#v\nhave %#+v", want, have)
			}
		})
	}

	{
		...
		if want, have := "alpha-default", inflags.Alpha; want != have {
			t.Errorf("alpha: want %q, have %q", want, have)
		}
		if want, have := 0, inflags.Beta; want != have {
			t.Errorf("beta: want %v, have %v", want, have)
		}
		if want, have := false, inflags.Delta; want != have {
...

	{
		...
		if want, have := "foo", inflags.Alpha; want != have {
			t.Errorf("alpha: want %q, have %q", want, have)
		}
		if want, have := 7, inflags.Beta; want != have {
			t.Errorf("beta: want %v, have %v", want, have)
		}
		if want, have := true, inflags.Delta; want != have {
			t.Errorf("delta: want %v, have %v", want, have)
		}
	}
	...
	t.Run("implements", func(t *testing.T) {
		...
		fs := flags.NewFlagSet(t.Name())
		if err := fs.AddStruct(&myflags); err != nil { // should allow
		...
		if want, have := []string{"a", "b"}, myflags.Foo.Get(); !reflect.DeepEqual(want, have) {
...
func TestFlagSet_Std(t *testing.T) {
	...
	fs := flags.NewFlagSetFrom(stdfs.Name(), stdfs)
...
func TestFlagSet_NonExistingAllowed(t *testing.T) {
	t.Parallel()
	...
	for _, argstr := range []string{
		...
		t.Run(argstr, func(t *testing.T) {
			fs := flags.NewFlagSet("nonexist")
			fs.Bool('b', "boolean", "boolean flag")
			fs.StringLong("string", "default", "string flag")
			fs.Duration('d', "duration", 250*time.Millisecond, "duration flag")
			ValidateFlags(t, fs, strings.Fields(argstr), fs.WithArgsIgnoreUndefinedFlags())
		})
	}
}
