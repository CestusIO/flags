...
type FlagSet struct {
	...
	parent                   *FlagSet
	argsIgnoreUndefinedFlags bool
}

var _ ff.Flags = (*FlagSet)(nil)
var _ ff.Resetter = (*FlagSet)(nil)
...
func NewFlagSet(name string) *FlagSet {
	return &FlagSet{
		...
		isStdAdapter:             false,
		parent:                   nil,
		argsIgnoreUndefinedFlags: false,
	}
}
...
// ApplyOptions is a clutch to allow setting options on a flagset (other then trough the parse method)
// It is usefull in conjunction with the ff.Command since options are global but subcommands can have their own
// flagset. So things like argsIgnoreUndefinedFlags will get lost
func (fs *FlagSet) ApplyOptions(options ...ff.Option) {
	var pc ff.ParseContext
...
func (fs *FlagSet) Parse(args []string) error {
	...
	case errors.Is(err, ErrUnknownFlag):
		if fs.argsIgnoreUndefinedFlags {
			err = nil
			fs.isParsed = true
		} else {
...
func (fs *FlagSet) parseArgs(args []string) (err error) {
	...
	for len(args) > 0 {
		...
		if parseDone {
			return // fs.postParseArgs should include arg
		}
...
func (fs *FlagSet) parseShortFlag(arg string, args []string) ([]string, error) {
	...
	for i, r := range arg {
		...
		if f == nil {
			...
			case r == 'h':
				return args, ErrHelp
			case fs.argsIgnoreUndefinedFlags:
				// we have to skip the flag and continue parsing
...
func (fs *FlagSet) WalkFlags(fn func(ff.Flag) error) error {
	for cursor := fs; cursor != nil; cursor = cursor.parent {
...
func (fs *FlagSet) GetFlag(name string) (ff.Flag, bool) {
	if name == "" {
		return nil, false
	}
...
func (fs *FlagSet) AddFlag(cfg FlagConfig) (ff.Flag, error) {
	if fs.isStdAdapter {
...
func (fs *FlagSet) Value(short rune, long string, value flag.Value, usage string) ff.Flag {
...
func (fs *FlagSet) ValueShort(short rune, value flag.Value, usage string) ff.Flag {
	return fs.Value(short, "", value, usage)
}

// ValueLong defines a new flag in the flag set, and panics on any error.
func (fs *FlagSet) ValueLong(long string, value flag.Value, usage string) ff.Flag {
	return fs.Value(0, long, value, usage)
}

// BoolVar defines a new default false bool flag in the flag set, and panics on
// any error.
func (fs *FlagSet) BoolVar(pointer *bool, short rune, long string, usage string) ff.Flag {
...
// BoolVarDefault.
func (fs *FlagSet) BoolVarDefault(pointer *bool, short rune, long string, def bool, usage string) ff.Flag {
...
func (fs *FlagSet) StringVar(pointer *string, short rune, long string, def string, usage string) ff.Flag {
	return fs.Value(short, long, ffval.NewValueDefault(pointer, def), usage)
}
...
func (fs *FlagSet) StringListVar(pointer *[]string, short rune, long string, usage string) ff.Flag {
...
func (fs *FlagSet) StringSetVar(pointer *[]string, short rune, long string, usage string) ff.Flag {
...
func (fs *FlagSet) StringEnumVar(pointer *string, short rune, long string, usage string, valid ...string) ff.Flag {
...
func (fs *FlagSet) Float64Var(pointer *float64, short rune, long string, def float64, usage string) ff.Flag {
	return fs.Value(short, long, ffval.NewValueDefault(pointer, def), usage)
}
...
func (fs *FlagSet) IntVar(pointer *int, short rune, long string, def int, usage string) ff.Flag {
	return fs.Value(short, long, ffval.NewValueDefault(pointer, def), usage)
}
...
func (fs *FlagSet) UintVar(pointer *uint, short rune, long string, def uint, usage string) ff.Flag {
	return fs.Value(short, long, ffval.NewValueDefault(pointer, def), usage)
}
...
func (fs *FlagSet) Uint64Var(pointer *uint64, short rune, long string, def uint64, usage string) ff.Flag {
	return fs.Value(short, long, ffval.NewValueDefault(pointer, def), usage)
}
...
// WithArgsIgnoreUndefinedFlags tells [Parse] to ignore flags in arguments
// which are not defined in the parsed flag set. This option only applies to
// flags in command line arguments.
//
// By default, undefined flags in commandline arguments result in a parse error.
func (fs *FlagSet) WithArgsIgnoreUndefinedFlags() ff.Option {
	return func(pc *ff.ParseContext) {
		fs.argsIgnoreUndefinedFlags = true
	}
}
...
var _ ff.Flag = (*coreFlag)(nil)
var _ ff.Resetter = (*coreFlag)(nil)

func (f *coreFlag) GetFlags() ff.Flags {
	return f.flagSet
}
...
func (f *coreFlag) Reset() error {
	if r, ok := f.flagValue.(ff.Resetter); ok {
